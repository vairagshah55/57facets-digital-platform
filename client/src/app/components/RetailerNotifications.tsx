...
import {
  Bell, Truck, Sparkles, Megaphone, CheckCheck, Loader2, Trash2,
} from "lucide-react";
...
const TYPE_CFG: Record<string, { icon: React.ReactNode; color: string; bg: string; label: string }> = {
  "order-update":   { icon: <Truck className="w-4 h-4" />, color: "#a855f7", bg: "rgba(168,85,247,0.12)", label: "Order" },
  "new-collection": { icon: <Sparkles className="w-4 h-4" />, color: "var(--sf-teal)", bg: "rgba(48,184,191,0.12)", label: "Product" },
  announcement:     { icon: <Megaphone className="w-4 h-4" />, color: "#f59e0b", bg: "rgba(245,158,11,0.12)", label: "Announcement" },
  system:           { icon: <Bell className="w-4 h-4" />, color: "#3b82f6", bg: "rgba(59,130,246,0.12)", label: "System" },
};
...
export function RetailerNotifications() {
  ...
  const markRead = async (id: string) => {
    try { await notifApi.markRead(id); setNotifications((p) => p.map((n) => n.id === id ? { ...n, is_read: true } : n)); } catch {}
  };
  const markAllRead = async () => {
    try { await notifApi.markAllRead(); setNotifications((p) => p.map((n) => ({ ...n, is_read: true }))); } catch {}
  };
    ...
    <div className="mx-auto max-w-3xl px-4 sm:px-6 py-6 space-y-5">
      ...
      <motion.div {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.05 }} className="flex gap-2 flex-wrap">
        {[
          ...
          { key: "order-update", label: "Orders" },
          { key: "new-collection", label: "Products" },
