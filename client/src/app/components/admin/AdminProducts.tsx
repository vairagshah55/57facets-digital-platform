...
function formToPayload(f: ProductFormData) {
  const payload: Record<string, any> = {
    ...
    occasion_tags: Array.isArray(f.occasion_tags) ? f.occasion_tags : (f.occasion_tags ? f.occasion_tags.split(",").map((t: string) => t.trim()).filter(Boolean) : []),
    ...
    gold_purity_options: Array.isArray(f.gold_purity_options) ? f.gold_purity_options : (f.gold_purity_options ? f.gold_purity_options.split(",").map((t: string) => t.trim()).filter(Boolean) : []),
    finish_options: Array.isArray(f.finish_options) ? f.finish_options : (f.finish_options ? f.finish_options.split(",").map((t: string) => t.trim()).filter(Boolean) : []),
