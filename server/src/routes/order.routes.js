...
// Retailer cancels their OWN order — only while it is still 'pending'. Once the
// admin moves it past pending (confirmed/processing/...), only the admin can act.
router.post("/:id/cancel", async (req, res, next) => {
  ...
  try {
    ...
    if (order.status !== "pending") {
      throw new AppError("This order can no longer be cancelled — please contact us.", 403);
    }
    ...
    await client.query(
      `INSERT INTO order_tracking (order_id, status, detail)
       VALUES ($1, 'Cancelled', 'Order cancelled by retailer')`,
      [order.id]
    );

    // Notify retailer + admin
    await client.query(
      `INSERT INTO notifications (retailer_id, type, title, message, action_path)
       VALUES ($1, 'order-update', 'Order Cancelled', $2, '/retailer/orders')`,
      [req.retailer.id, `${order.order_number} has been cancelled.`]
    );
    await client.query(
      `INSERT INTO admin_notifications (type, title, message, action_path)
       VALUES ('order', $1, $2, '/admin/orders')`,
      [`Order Cancelled: ${order.order_number}`, `${req.retailer.name || "A retailer"} cancelled order ${order.order_number}.`]
    );
