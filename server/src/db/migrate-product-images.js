...
// Resized WebP variants generated by backfill-image-variants.js / imageVariants.js.
const VARIANT_RE = /_(thumb|card|full|blur)\.webp$/i;
...
async function run() {
  ...
  try {
    ...
    for (const [folder, categoryName] of Object.entries(FOLDER_CATEGORY_MAP)) {
      ...
      // Get all image files in this folder. Skip the resized variants that
      // backfill-image-variants.js writes alongside each original, otherwise a
      // re-run imports them as products images and variants get built of variants.
      const files = fs.readdirSync(folderPath).filter(
        (f) => /\.(jpe?g|png|webp|avif)$/i.test(f) && !VARIANT_RE.test(f)
      );
